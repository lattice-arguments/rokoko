...
Valid log levels to be set for RUST_LOG are (all case insensitive) `debug`, `warn` or `warning`, `err` or `error`, and `info`, with `info` being the default. Note that other syntaxes or numeric values are not supported, as the env is not read through `tracing-subscriber` with the `env_filter` feature, so additional options supported in that case are ignored.
