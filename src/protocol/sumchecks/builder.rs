...
pub fn init_sumcheck(crs: &crs::CRS, config: &SumcheckConfig) -> SumcheckContext {
    ...
    // Both sides share the combined witness; FactoredDiffSumcheck pulls it
    // out of the per-point loop, saving one polynomial multiplication per
    // half-hypercube point.
